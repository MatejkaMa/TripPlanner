...
final class FlightConnectionsListViewModel: ObservableObject {
  ...
  init(
    _ connections: [FlightConnection], from: AnyPublisher<City?, Never>,
    to: AnyPublisher<City?, Never>, onSelect: @escaping (CityConnection) -> Void
  ) {
          ...
          .map { path in
            let children: [CityConnection] = {
              return path.connections.reversed().reduce(into: [CityConnection]()) {
                partialResult, nodeConnection in
                if let last = partialResult.last {
                  partialResult += [
                    CityConnection(
                      from: last.to, to: nodeConnection.to.item, price: nodeConnection.weight,
                      children: nil)
                  ]
                } else {
                  partialResult = [
                    CityConnection(
                      from: departure, to: nodeConnection.to.item, price: nodeConnection.weight,
                      children: nil)
                  ]
                }
              }
            ...
            return [
              CityConnection(
                from: departure, to: destination, price: path.cumulativeWeight, children: children)
            ]
          } ?? []
